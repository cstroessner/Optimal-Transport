...
function [P, err, usedIters, success] = MMSinkhorn4(C, a, b, c, d, lambda, maxIters, tol, disableWarnings)
...
% Sinkhorn iterations
for iter = 1:maxIters
    P = computeP(x1,x2,x3,x4,A); %Compute Tensor 3.9
    [s1,s2,s3,s4] = computeMarginals(P);
...
function P = computeP(b1,b2,b3,b4,K)
n = size(K);
P = K;
for i = 1:n(1)
   for j = 1:n(2)
       for k = 1:n(3)
           for l = 1:n(4)
                P(i,j,k,l) = P(i,j,k,l) * exp(b1(i)+b2(j)+b3(k)+b4(l));
           end
       end
   end  
end
...
function [r1,r2,r3,r4] = computeMarginals(P)
r1 = squeeze(sum(P,[2,3,4]));
r2 = squeeze(sum(P,[1,3,4]))';
r3 = squeeze(sum(P,[1,2,4]));
r4 = squeeze(sum(P,[1,2,3]));
end
